...
dataset = "Synapse"
# base_dir = join("/root/autodl-tmp/AgileFormer/segm_visualization/Synapse_modulated", dataset)
base_dir = "/root/autodl-tmp/AgileFormer/segm_visualization/Synapse_modulated"
...
methods = ['swinunet', 'Hiformer', 'missformer', 'perspective_unet', 'AgileFormer_mPatchEmbedding_DSC']

# color_pattles = ['red','cyan','yellow','forestgreen','blue','purple','darkorange','deeppink']
color_pattles = ['mediumaquamarine', 'palegoldenrod', 'mediumpurple', 'lightcoral', 'skyblue', 'forestgreen', 'orange', 'purple']
...
#methods = ['swinunet', 'Hiformer', 'missformer', 'perspective_unet', 'AgileFormer_mPatchEmbedding_DSC']
    for c in candiates:
        ...
        segm_swinunet_overlay = overlay_masks(image[..., s], prepare_mask(segm_predictions[0, ..., s], classes),
                                              colors=colors, alpha=alpha, beta=beta)
        segm_hiformer_overlay = overlay_masks(image[..., s], prepare_mask(segm_predictions[1, ..., s], classes), colors=colors, alpha=alpha, beta=beta)
        segm_missformer_overlay = overlay_masks(image[..., s], prepare_mask(segm_predictions[2, ..., s], classes), colors=colors, alpha=alpha, beta=beta)
        segm_perspectiveunet_overlay = overlay_masks(image[..., s], prepare_mask(segm_predictions[3, ..., s], classes), colors=colors, alpha=alpha, beta=beta)
        # segm_agile_overlay = overlay_masks(image[..., s], prepare_mask(segm_predictions[0, ..., s], classes), colors=colors, alpha=alpha, beta=beta)
        # segm_agile_mPE_overlay = overlay_masks(image[..., s], prepare_mask(segm_predictions[0, ..., s], classes), colors=colors, alpha=alpha, beta=beta)
        # segm_agile_DSC_overlay = overlay_masks(image[..., s], prepare_mask(segm_predictions[3, ..., s], classes),colors=colors, alpha=alpha, beta=beta)
        segm_agile_mPatchEmbedding_DSC_overlay = overlay_masks(image[..., s], prepare_mask(segm_predictions[4, ..., s], classes),colors=colors, alpha=alpha, beta=beta)
        ...
        # image_grid = np.concatenate([segm_gt_overlay,segm_hiformer_overlay, segm_agile_overlay,segm_agile_mPE_overlay, segm_agile_DSC_overlay, segm_agile_mPatchEmbedding_DSC_overlay], axis=1)
        image_grid = np.concatenate([segm_gt_overlay,segm_swinunet_overlay,segm_hiformer_overlay,segm_missformer_overlay,segm_perspectiveunet_overlay,segm_agile_mPatchEmbedding_DSC_overlay], axis=1)
