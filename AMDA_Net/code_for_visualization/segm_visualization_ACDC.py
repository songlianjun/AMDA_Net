...
dataset = "ACDC"
base_dir = '/root/autodl-tmp/AgileFormer/segm_visualization/ACDC'
save_dir = join(base_dir, "visualization_AgileFormer")
...
methods = ['predictions_perspectivenet','predictions_swinunet', 'mPEDSC']
color_pattles = ['red', 'green', 'blue']
...
for gt_path in subfiles(join(base_dir, "segm")):
    ...
    for c in candiates:
        ...
        # segm_baseline_overlay = overlay_masks(image[..., s], prepare_mask(segm_predictions[0, ..., s], classes), colors=colors, alpha=alpha, beta=beta)
        segm_predictions_perspectivenet_overlay = overlay_masks(image[..., s], prepare_mask(segm_predictions[0, ..., s], classes), colors=colors, alpha=alpha, beta=beta)
        segm_predictions_swinunet_overlay = overlay_masks(image[..., s], prepare_mask(segm_predictions[1, ..., s], classes), colors=colors, alpha=alpha, beta=beta)
        segm_mPEDSC_overlay = overlay_masks(image[..., s], prepare_mask(segm_predictions[2, ..., s], classes), colors=colors, alpha=alpha, beta=beta)
        # segm_swin_overlay = overlay_masks(image[..., s], prepare_mask(segm_predictions[4, ..., s], classes), colors=colors, alpha=alpha, beta=beta)

        image_grid = np.concatenate([segm_gt_overlay, segm_predictions_perspectivenet_overlay, segm_predictions_swinunet_overlay,segm_mPEDSC_overlay], axis=1)
